...
---
This R Markdown file outlines the primary analyses conducted on the tradeoff model runs completed in "3_Model_Runs.Rmd"

It is broadly separated into the following sections:
- First, loading the raw data outputs from the tradeoff model, combining results for maize and soy into a single binary conversion recommendation for each model run, and then combining all biodiversity index runs into a single data.table for each model weighting specification. These data.tables have one row for each pixel in Zambia (1 km2 resolution), and one column for each model run, with either a 1 or 0 representing the model's recommendation to convert that pixel (or not).
...
1. Existing biodviersity indices
...
# $convertible is the proportion of each cell that is "unconverted to cropland or urban." This determines how much area each pixel could supply, and therefore how much food  the pixel can create if it's converted to agriculture. See 1_Start chunk "load-basic-gis-files" for inputs. 
...
# plots
Figure 1 - ppt methods schematic
Figure 2 - ensemble rasters (chunk: "*ms_fig_2_raster_bp")
Figure 3 - jaccard similarity (chunk: "*ms_fig_jac_w_bp")
Figure 4 - comparing production levels (5_SI.rmd, chunk: "z50-bp-comparison", file called robustness check)
...
# save:
png(paste0(p_plots, "/ms_final/", "ms_fig_3_jac_w_bp", ".png"), 
...
png(paste0(p_plots, "/ms_final/", "ms_fig_4_robustness", ".png"),
    width = 7, height = 4, units = "in", res = 400)
# png(paste0(p_plots, "/ms_final/", "ms_fig_4_robustness_SI", ".png"), # for SI
#     width = 7, height = 4, units = "in", res = 400)
...
gg_agreement_base <- ggplot(data = facet_stats) +
  theme_classic() + 
  theme(axis.text.x = element_text(angle = 320, vjust = 1, hjust = 0)) +
  xlab("Factor") + ylab("Mean Cell Value")
...
# start png call here. 
# bp weights here:
png(paste0(p_plots, "/ms_final/","ms_fig_2_facet_r_", weight, "_wide.png"),
    width = 8, height = 5,
    units = "in", res = 300)

# for z50, all
# png(paste0(p_plots, "/ms_final/","SI_fig_facet_r_", weight, "_wide.png"),    
#     width = 8, height = 5,
#     units = "in", res = 300)
...
# make list to run through the save_thumbnails() function
panel_rasters <- list(
...
# Save thumbnail images for methods panel: 
