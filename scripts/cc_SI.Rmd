...
Note: values are calculated by first calculating richness within each taxonomic group, normalizing to between 0-1, then averaging across all four taxonomic groups, following @Brancalion2019. See Section \@ref(section-norm-order) for a detailed comparison of this approach vs. the opposite, taken by @Laurance2014a.
...
### Processing spatial data and producing biodviersity indices
...
Each of the following figures shows, from top to bottom: the distribution of values in each layer, the raw biodiveristy inputs provided to the model, and conversion maps at 3 model specifications: *bp* (100% weight on biodiversity, average yields, baseline production target of ~5% of Zambia), *b* (100% weight on biodiversity, but with heterogeneous yields), and *z50* (100% weight on biodiversity, average yields, but with a production target resulting in conversion of ~50% of Zambia).
